...
离线里程计、位姿图和学习消融主结果自 2026-09-14 起采用不复用 RGB、depth 或 groundtruth 样本的一对一关联，fr1/xyz 共 790 帧。下文单独标为“历史运行”的 796 帧 ROS2 结果使用早期最近邻复用协议，不与 790 帧主结果比较。
...
```mermaid
flowchart LR
    T["fr1/desk\n573 帧"] -->|"训练 1216 个困难负例三元组"| M["MobileNetV3 128D 描述子\n无 SE / 有 SE，3 个种子"]
    V["fr1/desk2\n612 帧"] -->|"选择各方法相似度阈值和最佳 epoch"| M
    V --> B["ImageNet 冻结 576D 基线\n不做任务训练"]
    M --> X["冻结模型和阈值"]
    B --> S["fr1/xyz\n790 帧最终测试"]
    X --> S
...
三种子结果显示，冻结 ImageNet 特征的测试 Recall@1 和下游位姿图 ATE 均优于两种任务训练模型；SE 只略微提高平均 pair F1，没有提高 Recall@K 或最终 ATE。因此深度支路仍是实验模块，不能表述为已经优于简单预训练基线。
